...
if (isset($_POST['order_btn'])) {
   ...
   if (mysqli_num_rows($query1) > 0) {
       while ($fetch_cart_outer = mysqli_fetch_assoc($query1)) {
           $prod_name = $fetch_cart_outer['name'];
           $price = $fetch_cart_outer['price'];
           $quantity = $fetch_cart_outer['quantity'];
           $image = $fetch_cart_outer['image'];
           ...
           do {
               $transaction_id = time() . mt_rand(100000, 999999);
               $check_query = mysqli_query($con, "SELECT COUNT(*) as count FROM orders WHERE transaction_id = '$transaction_id'");
               $check_row = mysqli_fetch_assoc($check_query);
           } while ($check_row['count'] > 0);


           // Insert order details into the database
           $order = mysqli_query($con, "INSERT INTO orders(transaction_id, customer_id, image, product_name, price, quantity, month, month_year ) VALUES('$transaction_id', '$userId', '$image', '$prod_name', '$price', '$quantity', '$monthdate', '$monthdateyear')") or die('query failed');


           // Update product stocks
           $select_cart_inner = mysqli_query($con, "SELECT * FROM `products` where product_name='$prod_name'");
           while ($fetch_cart_inner = mysqli_fetch_assoc($select_cart_inner)) {
               $stocks = $fetch_cart_inner['total_stocks'];
           }
   ...
   if ($query1 && $order) {
       ...
       <div class='order-message-container'>
           <div class='message-container'>
               <h3>Thank you for ordering! Your Transaction ID is: $transaction_id</h3>
